...
const Page = () => {
    ...
    const copyToClipboard = async () => {
        try {
            if ("data" in response) {
                await navigator.clipboard.writeText(`${process.env.NEXT_PUBLIC_APP_URL || "https://zapurl.vercel.app"}/${response.data.shortUrl || ""}`);
                setIsCopied(true);
        ...
        <section className="flex p-4 md:p-0 justify-center items-center gap-7 w-full flex-col min-h-[90vh] ">
            ...
            <footer className=" absolute bottom-0 left-0 right-0  w-full text-center justify-center items-start flex">
                <h1 className=" text-lg pb-4 flex gap-2 items-center">
                    ...
                    <a href="https://github.com/Drake-knight" target="_blank" className=" underline text-blue-500 font-bold">
                        Vishal
                    </a>{" "}
                </h1>
            </footer>
